...
from fastapi import APIRouter, HTTPException, Request, Response, status, WebSocket
from app.utils.websocket_utils import NotificationManager
...
@router.websocket(
    "/{user_id}/notifications/register",
)
async def register_for_notifications(websocket: WebSocket, user_id: str) -> dict:
    try:
        await manager.connect_user(user_id, websocket)
        msg = await websocket.receive_text()
        logger.info(f"Received message: {msg}")
        await manager.send_notification(user_id, "Registered for notifications.")
        await manager.broadcast("Sent to everyone.")
        logger.info(f"User {user_id} registered for notifications.")
        return {"message": "Registered for notifications."}
    except Exception as e:
        logger.error(f"Failed to register user {user_id} for notifications.")
        raise HTTPException(
            status_code=500, detail="Failed to register for notifications."
        )
